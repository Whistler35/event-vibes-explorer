...
const ResetPassword = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 relative">
      <Button type="button" variant="ghost" size="sm" onClick={() => navigate('/auth')} className="absolute top-4 left-4 text-muted-foreground hover:text-foreground">
      ...
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-bold text-foreground pt-2">{t('auth.newPasswordTitle')}</h1>
        </div>

        {!sessionReady ? (
          <p className="text-center text-muted-foreground text-sm">{t('auth.verifyingLink')}</p>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="new-password" className="text-foreground">{t('auth.newPassword')} *</Label>
              <div className="relative">
                <Input id="new-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required className="bg-card border-border text-foreground pr-10" />
            ...
            <div className="space-y-2">
              <Label htmlFor="confirm-new-password" className="text-foreground">{t('auth.confirmPassword')} *</Label>
              <Input id="confirm-new-password" type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className="bg-card border-border text-foreground" />
            </div>
