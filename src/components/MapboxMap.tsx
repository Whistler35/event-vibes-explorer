...
const MapboxMap = forwardRef<MapboxMapHandle, MapboxMapProps>(({
  ...
  useImperativeHandle(ref, () => ({
    ...
    setOverview: (lat: number, lng: number, zoomLevel?: number) => {
      const m = map.current;
      if (!m) return;
      m.stop();
      // Preserve current zoom when no zoom level is provided (city search → pan only).
      const targetZoom = typeof zoomLevel === 'number' ? zoomLevel : m.getZoom();
      m.jumpTo({ center: [lng, lat], zoom: targetZoom, pitch: 0, bearing: 0 });
    },
    getBounds: () => {
    ...
    <div style={{ height }} className="w-full overflow-hidden relative">
      ...
      {!isPlaceMode && isLoaded && (
        <button
          ...
          style={{ bottom: `calc(280px + 1rem + env(safe-area-inset-bottom))` }}
          className="absolute right-5 z-30 w-12 h-12 bg-primary rounded-full flex items-center justify-center shadow-lg hover:bg-primary/90 transition-all"
